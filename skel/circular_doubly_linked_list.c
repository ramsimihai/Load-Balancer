...
dll_print_int_list(doubly_linked_list_t* list)
{
	...
	// checks if the list isn't allocated
	if (list == NULL)
		return;

	// initializes a node with the head of the list
	curr = list->head;

	// iterates through the list and print every node's data
...
dll_move_first(dll_node_t **head, dll_node_t *new_node)
{
	...
	// checks if the list is empty, then add the node on the first position;
	if (*head == NULL) {
	...
	// links the next and prev of the new_node to the new node;
	new_node->next->prev = new_node->prev;
	new_node->prev->next = new_node->next;

	last = (*head)->prev;

	// links the new node next and prev to the head and the last node
	new_node->next = *head;
	new_node->prev = last;
